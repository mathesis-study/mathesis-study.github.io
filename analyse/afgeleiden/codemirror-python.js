(() => {
  ...
  // codemirror-python.src.js
  var catalogus = {
    math: ["pi", "e", "sqrt", "sin", "cos", "tan", "log", "log10", "exp", "floor", "ceil", "degrees", "radians"],
  ...
  var sleutelwoorden = ["if", "else", "elif", "for", "while", "def", "return", "import", "from", "as", "class", "try", "except", "finally", "with", "in", "and", "or", "not", "True", "False", "None"];
  var ingebouwd = ["print", "input", "range", "len", "int", "float", "str", "list", "min", "max", "sum", "enumerate", "abs", "round", "zip"];
  var kleuren = HighlightStyle.define([
    { tag: tags.keyword, color: "var(--kleur-accent)" },
    { tag: tags.string, color: "var(--kleur-goed)" },
    { tag: tags.number, color: "var(--kleur-secundair, var(--kleur-accent))" },
    { tag: tags.comment, color: "var(--kleur-zwak)" },
    { tag: tags.function(tags.variableName), color: "var(--kleur-accent)" },
    { tag: tags.operatorKeyword, color: "var(--kleur-tekst)" }
  ]);
  function woordVoor(tekst, pos) {
    const m = /[A-Za-z_][A-Za-z0-9_]*$/.exec(tekst.slice(0, pos));
    return m ? m[0] : "";
  }
  function inCommentaarOfString(tekst, pos) {
    const regel = tekst.slice(tekst.lastIndexOf("\n", pos - 1) + 1, pos);
    let quote = null;
    for (let i2 = 0; i2 < regel.length; i2++) {
      const c = regel[i2];
      if (!quote && c === "#") return true;
      if ((c === "'" || c === '"') && regel[i2 - 1] !== "\\") quote = quote === c ? null : quote || c;
    }
    return Boolean(quote);
  }
  function lokaleNamen(tekst) {
    const namen = /* @__PURE__ */ new Set();
    const modules = {};
    for (const match of tekst.matchAll(/^\s*(?:def|class)\s+([A-Za-z_]\w*)|^\s*([A-Za-z_]\w*)\s*=|^\s*import\s+(math|random|statistics)(?:\s+as\s+([A-Za-z_]\w*))?|^\s*from\s+(math|random|statistics)\s+import\s+([A-Za-z_]\w*)(?:\s+as\s+([A-Za-z_]\w*))?/gm)) {
      if (match[1] || match[2]) namen.add(match[1] || match[2]);
      if (match[3]) {
        modules[match[4] || match[3]] = match[3];
        namen.add(match[4] || match[3]);
      }
      if (match[5]) namen.add(match[7] || match[6]);
    }
    return { namen: [...namen], modules };
  }
  function opties(namen, prefix, type) {
    return namen.filter((n) => n.startsWith(prefix)).sort().map((label) => ({ label, type }));
  }
  function voorstellen(tekst, pos) {
    if (inCommentaarOfString(tekst, pos)) return [];
    const voor = tekst.slice(0, pos), prefix = woordVoor(tekst, pos), lokaal = lokaleNamen(tekst);
    let m = /from\s+(math|random|statistics)\s+import\s+([A-Za-z_]\w*)?$/.exec(voor);
    if (m) return opties(catalogus[m[1]], prefix, "function");
    m = /(?:from|import)\s+([A-Za-z_]\w*)$/.exec(voor);
    if (m) return opties(Object.keys(catalogus), prefix, "module");
    m = /([A-Za-z_]\w*)\.([A-Za-z_]\w*)?$/.exec(voor);
    if (m && lokaal.modules[m[1]]) return opties(catalogus[lokaal.modules[m[1]]], prefix, "function");
    return [...opties(lokaal.namen, prefix, "variable"), ...opties(ingebouwd, prefix, "function"), ...opties(sleutelwoorden, prefix, "keyword")];
  }
  function bronCompletion(context) {
    const tekst = context.state.doc.toString(), pos = context.pos, prefix = woordVoor(tekst, pos);
    const result = voorstellen(tekst, pos);
    if (!result.length && !context.explicit) return null;
    return { from: pos - prefix.length, options: result, filter: false };
  }
  function foutNaarDiagnose(fout, doc2) {
    if (!fout || !fout.regel) return [];
    const regel = Math.max(1, Math.min(fout.regel, doc2.lines));
    const line = doc2.line(regel);
    const kolom = Math.max(0, Math.min((fout.kolom || 1) - 1, line.length));
    const eind = Math.max(kolom + 1, Math.min((fout.eindkolom || fout.kolom || 1) - 1, line.length));
    return [{ from: line.from + kolom, to: line.from + eind, severity: "error", message: fout.soort ? fout.soort + ": " + fout.bericht : fout.bericht || "Pythonfout" }];
  }
  var markeerEffect = StateEffect.define();
  var markeerVeld = StateField.define({
    create: () => Decoration.none,
    update(waarde, transactie) {
      waarde = waarde.map(transactie.changes);
      for (const effect of transactie.effects) {
        if (!effect.is(markeerEffect)) continue;
        if (!effect.value) return Decoration.none;
        const doc2 = transactie.state.doc, lijn = Decoration.line({ class: "cm-volgende-regel" });
        const van = Math.max(1, effect.value.van), tot = Math.min(doc2.lines, effect.value.tot);
        const bereiken = [];
        for (let n = van; n <= tot; n++) bereiken.push(lijn.range(doc2.line(n).from));
        return Decoration.set(bereiken);
      }
      return waarde;
    },
    provide: (veld) => EditorView.decorations.from(veld)
  });
  var markeerInvoerEffect = StateEffect.define();
  var markeerInvoerVeld = StateField.define({
    create: () => Decoration.none,
    update(waarde, transactie) {
      waarde = waarde.map(transactie.changes);
      for (const effect of transactie.effects) {
        if (!effect.is(markeerInvoerEffect)) continue;
        if (!effect.value) return Decoration.none;
        const doc2 = transactie.state.doc, positie = effect.value;
        const beginRegel = Math.max(1, Math.min(positie.regel, doc2.lines));
        const eindRegel = Math.max(beginRegel, Math.min(positie.eindregel || beginRegel, doc2.lines));
        const beginLijn = doc2.line(beginRegel), eindLijn = doc2.line(eindRegel);
        const begin = beginLijn.from + Math.max(0, Math.min(positie.kolom || 0, beginLijn.length));
        const eind = eindLijn.from + Math.max(0, Math.min(positie.eindkolom || 0, eindLijn.length));
        return Decoration.set([Decoration.mark({ class: "cm-python-invoer" }).range(begin, Math.max(begin + 1, eind))]);
      }
      return waarde;
    },
    provide: (veld) => EditorView.decorations.from(veld)
  });
  window.MathesisCodeMirror = {
    maakEditor(opties2) {
      const diagnostics = new Compartment();
      const view = new EditorView({ parent: opties2.parent, state: EditorState.create({ doc: opties2.value || "", extensions: [
        lineNumbers(),
        ...
        indentOnInput(),
        python(),
        syntaxHighlighting(kleuren),
        keymap.of([...defaultKeymap, indentWithTab, { key: "Shift-Tab", run: indentLess }, ...completionKeymap]),
        autocompletion({ override: [bronCompletion], activateOnTyping: false }),
        diagnostics.of(linter(() => [])),
        markeerVeld,
        markeerInvoerVeld,
        EditorView.updateListener.of((update) => {
          if (update.docChanged) opties2.onChange && opties2.onChange();
        }),
        EditorView.contentAttributes.of({ "aria-label": opties2.label || "Pythoncode", "aria-description": "Gebruik Tab om in te springen. Druk Escape en daarna Tab om de editor te verlaten. Ctrl+Spatie toont voorstellen." })
      ] }) });
      return { getValue: () => view.state.doc.toString(), setValue(tekst) {
        view.dispatch({ changes: { from: 0, to: view.state.doc.length, insert: tekst } });
      }, focus: () => view.focus(), destroy: () => view.destroy(), clearDiagnostics() {
        view.dispatch({ effects: diagnostics.reconfigure(linter(() => [])) });
      }, setDiagnostic(fout) {
        view.dispatch(setDiagnostics(view.state, foutNaarDiagnose(fout, view.state.doc)));
      }, markeerRegels(van, tot) {
        view.dispatch({ effects: markeerEffect.of(van ? { van, tot: tot || van } : null) });
      }, markeerInvoer(positie) {
        view.dispatch({ effects: markeerInvoerEffect.of(positie) });
